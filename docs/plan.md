...
**Vikunja cron poll** — a Hatchet cron task runs every 15 minutes, polls the Vikunja API, finds tasks with agent labels, pushes a Hatchet event per task:

```python
@hatchet.cron("*/15 * * * *")
async def poll_vikunja(ctx):
    tasks = vikunja.get_tasks(label_id=14, done=False)  # ai-research=14
    for task in tasks:
        await hatchet.event.push("agent:research", {
            "task_id": task.id, "title": task.title, "description": task.description
        })
        vikunja.mark_in_progress(task.id)
```

**GitHub PR webhook** — Hatchet receives the GitHub webhook directly, dispatches to PR Reviewer worker. No intermediary.
...
Existing PostgreSQL gets new databases:
...
- "All manifests under `apps/` are generated by app-factory. Never hand-edit them."
- "To update an app's config: edit the TOML spec in app-factory, re-run `make create-app`, open a PR here."
...
### Phase 2 — Storage: Qdrant on Mac Mini core stack + new PostgreSQL databases

**Goal:** Qdrant running and reachable from k3s. PostgreSQL has `hatchet` and `mem0` databases ready for later phases.
...
- `QDRANT_API_KEY`, `HATCHET_POSTGRES_PASSWORD`, `MEM0_POSTGRES_PASSWORD` added to Bitwarden and synced to the relevant stacks

**What gets built:**

In `ansible-playbooks/` — new tasks (or new playbook `mac-mini-agent-platform.yml`):
- Create the Docker volume before Komodo deploys:
  ```yaml
  - community.docker.docker_volume:
      name: services_qdrant-data
      state: present
  ```
- Create PostgreSQL databases, users, and extensions on the existing instance:
  ```yaml
  - community.postgresql.postgresql_user:
      name: hatchet
      password: "{{ hatchet_postgres_password }}"
  ...
  - community.postgresql.postgresql_ext:
      db: hatchet
      name: vector
  # same block repeated for mem0
  ```
  Ansible `postgresql_*` modules are idempotent — safe to re-run.
...
In `komodo-dean-gitops/mac-mini-m4/postgres/init.sh` — add new databases for future fresh installs (keeps bootstrap script authoritative):
```bash
...
```
Run `CREATE EXTENSION IF NOT EXISTS vector` in both (also in init.sh).

**Execution order:** Run `run_ansible("mac-mini-agent-platform.yml", "mac_mini")` via MCP first (volume + DB creation), then commit the core compose change to `komodo-dean-gitops` so Komodo deploys the updated stack (Qdrant starts, finds its volume). `run_ansible` is idempotent — re-running it is safe.
...
4. `hatchet` and `mem0` databases exist in PostgreSQL with correct owners
5. Both databases have the `vector` extension installed
6. Komodo shows core stack healthy after deploy
...
**Pre-conditions:**
- Phase 2 complete (`hatchet` database exists and is healthy)
- `HATCHET_SERVER_SECRET`, `HATCHET_ENCRYPTION_KEY` created in Bitwarden

**What gets built:**

Hatchet needs a PostgreSQL database (on Mac Mini) and a server secret. Provision via MCP:

```
scaffold_app("hatchet", "stateless k3s workflow engine with PostgreSQL backend", "stateless")
# Edit app-factory/apps/hatchet.toml:
#   component: image ghcr.io/hatchet-dev/hatchet:latest, port 7077
#   secrets: HATCHET_SERVER_SECRET (generate=true), HATCHET_ENCRYPTION_KEY (generate=true)
#   database: name "hatchet", host "10.100.20.18"
provision_app("hatchet")   # secrets → BWS, hatchet DB created in Mac Mini PG, manifests generated
open_deploy_pr("hatchet", "phase-3: deploy Hatchet workflow engine")
```

Supplementary files (hand-maintained alongside generated manifests):
- Hatchet-specific env vars added to the ExternalSecret (server TLS, encryption config)

In `agent-platform` repo:
- `workers/stub/main.py` — minimal Hatchet worker: accepts `test:ping` event, logs payload, returns `{"status": "ok"}`
...
2. Manually push `test:ping` event via Hatchet API → run appears in UI with status `SUCCEEDED` and logged payload
3. Cron fires on schedule → run appears in UI automatically without manual trigger
4. Kill the stub worker pod mid-run → Hatchet retries after worker comes back up
5. Hatchet UI shows run history, logs, input/output for all past runs
6. ArgoCD shows Hatchet app synced and healthy
...
In `agent-platform` repo:
- `common/memory.py` — thin Mem0 client wrapper (reads `MEM0_BASE_URL` + `MEM0_API_KEY` from env)
- Two Hatchet workers added to `agent-platform.toml` as components: worker A (`test:mem-write`), worker B (`test:mem-read`)
...
2. Push `test:mem-write` event → worker A writes a fact → Hatchet run `SUCCEEDED`
3. Push `test:mem-read` event with same `agent_id` → worker B retrieves the fact written in step 2
4. Push 10 concurrent `test:mem-write` events → all 10 runs succeed, all 10 facts are retrievable (concurrent write test)
5. Mem0 MCP server endpoint is reachable and lists `add_memory` / `search_memory` tools
6. ArgoCD shows Mem0 app synced and healthy
...
- `agent.py` — PydanticAI agent with tools:
  ...
  - `write_memory(content, agent_id)` → calls Mem0
  - `update_vikunja_task(task_id, status, comment)` → calls Vikunja API
- `worker.py` — Hatchet worker handling `agent:research` events
- `Dockerfile`
- Hatchet cron at startup: every 15 min → `GET /api/v1/tasks?label_id=14&done=false` → push `agent:research` event per task, mark task in-progress immediately
...
**Ready conditions (all must pass):**
1. Create Vikunja task "Research: summarize recent k3s releases", label `ai-research`
2. Within 15 minutes: Hatchet UI shows `agent:research` run with status `SUCCEEDED`
...
5. Run the same task twice → second run is skipped or de-duped (no duplicate reports)
...
- `agent.py` — PydanticAI agent with tools:
  ...
  - `read_file(path)`, `write_file(path, content)`, `run_shell(cmd)` (sandboxed to scratch dir)
  - `read_memory(agent_id)` → reads research context from Mem0 if a `task-{id}` namespace exists
- `worker.py` — Hatchet worker handling `agent:code` events
- Vikunja poller extended: label 11 (`ai-go`) → pushes `agent:code` event
...
- `agent.py` — PydanticAI agent: HTTP smoke tests against staging URL, reports pass/fail
...
**Ready conditions (all must pass):**
1. Open a PR on any `amerenda` repo → Hatchet UI shows `github:pr_opened` run within 60 seconds
2. PR receives a review comment from the agent within 5 minutes of opening
3. Manually push `deploy:staging` event → QA run appears and completes
4. QA result (pass/fail + details) is posted as a PR comment or Vikunja comment
5. PR Reviewer handles a deleted PR mid-run gracefully (no unhandled exception)
6. QA handles an unreachable staging URL gracefully (clear failure message, not a hung run)
